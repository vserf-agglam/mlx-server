"""
Anthropic message converter - pass-through converter for Anthropic models.
"""
...
class AnthropicMessageConverter(BaseMessageConverter):
    """
    Message converter for Anthropic models.
    This is a pass-through converter as requested.
    """

    def convert_messages(self, messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """
        Pass through messages without conversion for Anthropic models.
        ...
        """
        logger.debug(f"Anthropic converter: passing through {len(messages)} messages")
        return messages

    def convert_tools(self, tools: list[dict[str, Any]] | None) -> list[dict[str, Any]] | None:
        """
        Pass through tools without conversion for Anthropic models.
        ...
        """
        if tools:
            logger.debug(f"Anthropic converter: passing through {len(tools)} tools")
        return tools